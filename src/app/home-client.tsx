...
export function HomeClient({ 
  ...
  const heroBadge = (language === "bn" ? settings["hero_badge_bn"] : settings["hero_badge_en"]) || (
    language === "bn" ? "মানবতার সেবায় নিবেদিত যুবসমাজ" : "Youth Dedicated to Humanity"
  );
  
  const heroHeading = (language === "bn" ? settings["hero_heading_bn"] : settings["hero_heading_en"]);
  const heroDesc = (language === "bn" ? settings["hero_desc_bn"] : settings["hero_desc_en"]) || (
    language === "bn" 
  ...
  const causesTitle = (language === "bn" ? settings["causes_title_bn"] : settings["causes_title_en"]) || (
    language === "bn" ? "যেখানে আপনার অনুদান ব্যবহৃত হয়" : "Where Your Support Goes"
  );
  const causesDesc = (language === "bn" ? settings["causes_desc_bn"] : settings["causes_desc_en"]) || (
    language === "bn" 
  ...
  const goalTitle = (language === "bn" ? settings["goal_title_bn"] : settings["goal_title_en"]) || (
    language === "bn" ? "আমাদের মূল উদ্দেশ্য ও লক্ষ্য" : "Our Core Mission & Focus"
  );
  const goalDesc = (language === "bn" ? settings["goal_desc_bn"] : settings["goal_desc_en"]) || (
    language === "bn" 
  ...
  const statVolunteersLabel = (language === "bn" ? settings["stat_volunteers_label_bn"] : settings["stat_volunteers_label_en"]) || (
    language === "bn" ? "পরিবারকে সরাসরি সহায়তা" : "Families Supported"
  );

  const statTrees = settings["stat_trees"] || "0";
  const statTreesLabel = (language === "bn" ? settings["stat_trees_label_bn"] : settings["stat_trees_label_en"]) || (
    language === "bn" ? "বৃক্ষরোপণ কর্মসূচি" : "Trees Planted"
  );

  const statStudents = settings["stat_students"] || "0";
  const statStudentsLabel = (language === "bn" ? settings["stat_students_label_bn"] : settings["stat_students_label_en"]) || (
    language === "bn" ? "শিক্ষার্থীকে মেধা বৃত্তি" : "Students Sponsored"
  );

  const heroImage = settings["home_hero_image"] || "/about-banner.jpg";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
            ...
            <div className="lg:col-span-5 space-y-8 z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-3.5 items-start sm:items-center pt-1">
                <Link href="/registration" className="bg-growth-green hover:bg-[#236026] text-white font-bold px-7 py-3.5 rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-growth-green/20 hover:shadow-xl hover:shadow-growth-green/30 hover:-translate-y-0.5 text-sm">
                  <HeartHandshake size={18} />
                  {language === "bn" ? "সদস্য হোন" : "Join Now"}
                </Link>
                <Link href="/about" className="bg-surface-container-lowest/90 hover:bg-surface-container-lowest text-foreground font-bold px-7 py-3.5 rounded-xl flex items-center gap-2 transition-all border border-border backdrop-blur-sm hover:-translate-y-0.5 shadow-xs text-sm">
              ...
              <div className="pt-2 flex items-center gap-3 text-xs border-t border-border/60">
                ...
                <div>
                  <p className="font-bold text-foreground">
                    {language === "bn" ? "বীরখাল সমাজ কল্যাণ যুব সংগঠন" : "Birkhal Youth Social Welfare Organization"}
                  </p>
                  <p className="text-[11px] text-growth-green font-semibold flex items-center gap-1.5">
                    ...
                    {language === "bn" ? "মাঠপর্যায়ে মানবসেবায় সার্বক্ষণিক নিবেদিত" : "Dedicated to Community Service"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 relative w-full">
              <div className="relative h-[440px] sm:h-[500px] lg:h-[540px] w-full rounded-xl overflow-hidden shadow-2xl group z-10 border border-border/80 bg-surface-container">
                ...
                <Image 
                  src={settings["home_hero_image"] || "/charity_banner.jpg"} 
                  alt="Birkhal Youth Members" 
                  fill 
                  priority
                  unoptimized={(settings["home_hero_image"] || "").startsWith("data:")}
                  ...
                  <div className="absolute top-4 left-4 z-20 bg-transparent border border-white/40 p-2.5 sm:p-3 rounded-xl shadow-lg flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:border-white/70 group/card animate-in fade-in slide-in-from-top-3">
                    ...
                    <div className="text-white">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-base sm:text-lg tracking-tight text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statVolunteers}</span>
                        <span className="w-2 h-2 rounded-full bg-white animate-pulse shadow-[0_0_8px_#ffffff]"></span>
                      </div>
                      <p className="text-[10px] sm:text-[11px] font-bold text-white leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statVolunteersLabel}</p>
                    </div>
                  </div>
                )}
                ...
                {settings["section_stats_enabled"] !== "false" && (
                  <div className="absolute top-20 sm:top-24 right-4 z-20 bg-transparent border border-white/40 p-2.5 sm:p-3 rounded-xl shadow-lg flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:border-white/70 group/card animate-in fade-in slide-in-from-right-3">
                    ...
                    <div className="text-white">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-base sm:text-lg tracking-tight text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statTrees}</span>
                      ...
                      <p className="text-[10px] sm:text-[11px] font-bold text-white leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statTreesLabel}</p>
                    </div>
                  </div>
                )}
                ...
                {settings["section_stats_enabled"] !== "false" && (
                  <div className="absolute bottom-4 left-4 right-4 sm:right-auto z-20 bg-transparent border border-white/40 p-2.5 sm:p-3 rounded-xl shadow-lg flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:border-white/70 group/card animate-in fade-in slide-in-from-bottom-3">
                    ...
                    <div className="text-white">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-base sm:text-lg tracking-tight text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statStudents}</span>
                        <span className="text-[9px] font-bold text-white bg-transparent px-2 py-0.5 rounded-full border border-white/40 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">
                          {language === "bn" ? "শিক্ষা" : "Education"}
                        </span>
                      </div>
                      <p className="text-[10px] sm:text-[11px] font-bold text-white leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{statStudentsLabel}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-surface-container-lowest dark:bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-xl border border-border group transition-all duration-300 flex flex-col">
              <div className="h-52 relative overflow-hidden bg-gray-100 dark:bg-gray-800">
                <Image 
                  src={settings["cause_1_image"] || "/ai_education.jpg"} 
                  alt="Education" 
                  fill 
                  unoptimized={(settings["cause_1_image"] || "").startsWith("data:")}
              ...
              <div className="p-6 space-y-3 flex-grow flex flex-col justify-between">
                <div>
                  <h4 className="font-bold text-lg text-foreground group-hover:text-growth-green transition-colors">
                    {(language === "bn" ? settings["cause_1_title_bn"] : settings["cause_1_title_en"]) || (
                      language === "bn" ? "সবার জন্য শিক্ষা ও উপকরণ" : "Education for All"
                    )}
                  </h4>
                  <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed mt-1.5">
                    {(language === "bn" ? settings["cause_1_desc_bn"] : settings["cause_1_desc_en"]) || (
                      language === "bn" ? "দরিদ্র ও মেধাবী শিক্ষার্থীদের মাঝে বিনামূল্যে বই, খাতা, স্কুল ড্রেস ও মেধা বৃত্তি বিতরণ।" : "Providing free textbooks, uniforms, and educational scholarships for underprivileged children."
                    )}
                  </p>
                </div>
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs font-bold text-growth-green">
                  <span>{language === "bn" ? "সরাসরি মাঠপর্যায়ে" : "Active Program"}</span>
                  <Link href="/donations" className="hover:underline flex items-center gap-1">
                    <span>{language === "bn" ? "অনুদান দিন" : "Support"}</span> &rarr;
                  </Link>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest dark:bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-xl border border-border group transition-all duration-300 flex flex-col">
              <div className="h-52 relative overflow-hidden bg-gray-100 dark:bg-gray-800">
                <Image 
                  src={settings["cause_2_image"] || "/ai_plantation.jpg"} 
                  alt="Plantation & Environment" 
                  fill 
                  unoptimized={(settings["cause_2_image"] || "").startsWith("data:")}
                  className="object-cover object-top group-hover:scale-105 transition-transform duration-500" 
                />
              ...
              <div className="p-6 space-y-3 flex-grow flex flex-col justify-between">
                <div>
                  <h4 className="font-bold text-lg text-foreground group-hover:text-growth-green transition-colors">
                    {(language === "bn" ? settings["cause_2_title_bn"] : settings["cause_2_title_en"]) || (
                      language === "bn" ? "বীরখাল সবুজায়ন ও পরিবেশ রক্ষা" : "Plantation & Green Ecology"
                    )}
                  </h4>
                  <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed mt-1.5">
                    {(language === "bn" ? settings["cause_2_desc_bn"] : settings["cause_2_desc_en"]) || (
                      language === "bn" ? "রাস্তার পাশে, শিক্ষা প্রতিষ্ঠানে ও গ্রামে ফলজ ও বনজ বৃক্ষরোপণ করে একটি সবুজ পরিবেশ গড়ে তোলা।" : "Mass tree plantation across village roads and institutions to ensure a sustainable green future."
                    )}
                  </p>
                </div>
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs font-bold text-impact-orange">
                  <span>{language === "bn" ? "চলমান উদ্যোগ" : "Green Initiative"}</span>
                  <Link href="/donations" className="hover:underline flex items-center gap-1">
                    <span>{language === "bn" ? "অনুদান দিন" : "Support"}</span> &rarr;
                  </Link>
                </div>
            ...
            <div className="bg-surface-container-lowest dark:bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-xl border border-border group transition-all duration-300 flex flex-col">
              <div className="h-52 relative overflow-hidden bg-gray-100 dark:bg-gray-800">
                <Image 
                  src={settings["cause_3_image"] || "/ai_medical.jpg"} 
                  alt="Healthcare" 
                  fill 
                  unoptimized={(settings["cause_3_image"] || "").startsWith("data:")}
                  className="object-cover object-top group-hover:scale-105 transition-transform duration-500" 
                />
              ...
              <div className="p-6 space-y-3 flex-grow flex flex-col justify-between">
                <div>
                  <h4 className="font-bold text-lg text-foreground group-hover:text-growth-green transition-colors">
                    {(language === "bn" ? settings["cause_3_title_bn"] : settings["cause_3_title_en"]) || (
                      language === "bn" ? "ফ্রি মেডিকেল ও চিকিৎসা সহায়তা" : "Healthcare & Medical Camps"
                    )}
                  </h4>
                  <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed mt-1.5">
                    {(language === "bn" ? settings["cause_3_desc_bn"] : settings["cause_3_desc_en"]) || (
                      language === "bn" ? "দরিদ্র ও অসহায় রোগীদের বিনামূল্যে ওষুধ বিতরণ, চিকিৎসা তহবিল ও অ্যাম্বুলেন্স জরুরি সহায়তা।" : "Providing free essential medicines, health checkup camps, and emergency health grants."
                    )}
                  </p>
                </div>
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs font-bold text-reliability-blue">
                  <span>{language === "bn" ? "মানবিক সহায়তা" : "Medical Fund"}</span>
                  <Link href="/donations" className="hover:underline flex items-center gap-1">
                    <span>{language === "bn" ? "অনুদান দিন" : "Support"}</span> &rarr;
                  </Link>
                </div>
            ...
            <div className="bg-surface-container-lowest dark:bg-surface-container-low rounded-xl overflow-hidden shadow-xs hover:shadow-xl border border-border group transition-all duration-300 flex flex-col">
              <div className="h-52 relative overflow-hidden bg-gray-100 dark:bg-gray-800">
                <Image 
                  src={settings["cause_4_image"] || "/ai_relief.jpg"} 
                  alt="Emergency Relief" 
                  fill 
                  unoptimized={(settings["cause_4_image"] || "").startsWith("data:")}
                  className="object-cover object-top group-hover:scale-105 transition-transform duration-500" 
                />
              ...
              <div className="p-6 space-y-3 flex-grow flex flex-col justify-between">
                <div>
                  <h4 className="font-bold text-lg text-foreground group-hover:text-growth-green transition-colors">
                    {(language === "bn" ? settings["cause_4_title_bn"] : settings["cause_4_title_en"]) || (
                      language === "bn" ? "বন্যা ও দুর্যোগকালীন ত্রাণ তহবিল" : "Disaster & Emergency Relief"
                    )}
                  </h4>
                  <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed mt-1.5">
                    {(language === "bn" ? settings["cause_4_desc_bn"] : settings["cause_4_desc_en"]) || (
                      language === "bn" ? "বন্যা, শীতবস্ত্র বিতরণ ও যেকোনো দুর্যোগে পানিবন্দী এবং বিপদগ্রস্ত মানুষের কাছে খাবার পৌঁছে দেওয়া।" : "Immediate relief packages, cooked food, and warm clothes distribution during natural calamities."
                    )}
                  </p>
                </div>
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs font-bold text-red-600">
                  <span>{language === "bn" ? "জরুরি তহবিল" : "Emergency Fund"}</span>
                  <Link href="/donations" className="hover:underline flex items-center gap-1">
                    <span>{language === "bn" ? "অনুদান দিন" : "Support"}</span> &rarr;
                  </Link>
                </div>
            ...
            <div className="md:col-span-8 bg-gradient-to-br from-growth-green/15 via-surface-container-lowest to-surface-container-lowest dark:from-growth-green/10 dark:via-surface-container-low dark:to-surface-container-low p-8 sm:p-10 rounded-xl relative overflow-hidden border border-border shadow-xs hover:border-growth-green/40 transition-all flex flex-col justify-between min-h-[340px]">
              <div className="space-y-4 max-w-xl z-10">
                ...
                <h3 className="font-headline-md text-2xl sm:text-3xl text-foreground font-extrabold tracking-tight">
                  {language === "bn" ? "শতভাগ আর্থিক স্বচ্ছতা ও জবাবদিহিতা" : "100% Financial Transparency & Trust"}
                </h3>
              ...
              <div className="pt-6 border-t border-border/80 flex flex-wrap items-center justify-between gap-4 z-10">
                <div className="flex items-center gap-2 text-xs font-bold text-growth-green">
                  ...
                  <span>{language === "bn" ? "স্বচ্ছতা আমাদের অঙ্গীকার" : "Verified Charity Records"}</span>
                </div>
                <Link href="/donations" className="text-xs font-bold text-foreground hover:text-growth-green flex items-center gap-1 transition-colors">
                  <span>{language === "bn" ? "অনুদান বিবরণী দেখুন" : "View Accounts"}</span> &rarr;
                </Link>
              </div>
            </div>
            ...
            <div className="md:col-span-4 bg-surface-container-lowest dark:bg-surface-container-low p-8 rounded-xl border border-border shadow-xs hover:border-reliability-blue/40 transition-all flex flex-col justify-between min-h-[340px]">
              <div className="space-y-4">
                ...
                <h3 className="font-headline-md text-xl sm:text-2xl text-foreground font-bold">
                  {language === "bn" ? "ঐক্য ও সহযোগিতা" : "Unity & Brotherhood"}
                </h3>
              ...
              <div className="pt-4 text-xs font-bold text-reliability-blue">
                {language === "bn" ? "একতাই আমাদের শক্তি" : "United for Positive Change"}
              </div>
            </div>
