        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-surface-container-low dark:bg-surface-container-lowest p-6 md:p-8 border border-border shadow-sm text-center rounded-xl">
            ...
            <h3 className="font-headline-md text-xl font-bold text-growth-green mb-1">
              {language === 'bn'
                ? 'বীরখাল সমাজ কল্যাণ যুব সংগঠন'
                : 'Birkhal Social Welfare Youth Organization'}
            </h3>
            <p className="text-xs text-on-surface-variant mb-6">
              {language === 'bn'
                ? 'একটি আদর্শ ও মানবিক সমাজ গঠনের প্রত্যয়ে'
                : 'Committed to humanity & sustainable youth development'}
            </p>
            ...
            <div className="space-y-4 text-left pt-4 border-t border-border">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-on-surface-variant">
                    {language === 'bn'
                      ? 'সকল কাজে সততা ও সমতার প্রতিফলন'
                      : 'Total integrity in funds & relief actions'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold text-xs text-foreground">
                    {language === 'bn' ? 'সদস্যপদ সুবিধা' : 'Member Privileges'}
                  </h4>
                  <p className="text-[11px] text-on-surface-variant">
                    {language === 'bn'
                      ? 'সিদ্ধান্ত গ্রহণ ও সামাজিক কার্যক্রমে প্রত্যক্ষ ভূমিকা'
                      : 'Direct participation in development drives'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold text-xs text-foreground">
                    {language === 'bn'
                      ? 'আইডি কার্ড ও সার্টিফিকেট'
                      : 'Official Recognition'}
                  </h4>
                  <p className="text-[11px] text-on-surface-variant">
                    {language === 'bn'
                      ? 'ভেরিফায়েড সদস্য নম্বর ও সম্মাননা'
                      : 'Verified member profile in directory'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-surface-container-low dark:bg-surface-container-lowest p-6 border border-border rounded-xl">
            <h4 className="font-bold text-xs text-growth-green uppercase tracking-wider mb-2">
              {language === 'bn'
                ? 'ভর্তি সংক্রান্ত জরুরি নির্দেশনা'
                : 'Application Guidelines'}
            </h4>
            <ul className="text-xs text-on-surface-variant space-y-2 list-disc list-inside leading-relaxed">
              <li>
                {language === 'bn'
                  ? 'ফর্মের সকল তথ্য সঠিক ও নির্ভুল দিন।'
                  : 'Ensure all personal information is accurate.'}
              </li>
              <li>
                {language === 'bn'
                  ? 'স্পষ্ট পাসপোর্ট সাইজের ছবি আপলোড করুন।'
                  : 'Upload a clear passport-size photograph.'}
              </li>
              <li>
                {language === 'bn'
                  ? 'এডমিন কর্তৃক ভেরিফিকেশনের পর সদস্য তালিকায় নাম যুক্ত হবে।'
                  : 'Your profile will appear in the directory upon admin approval.'}
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="border-b border-border pb-4 mb-6 flex items-center justify-between">
            <div>
              <h2 className="font-headline-lg text-2xl font-bold text-foreground">
                {language === 'bn'
                  ? 'সদস্য ভর্তি আবেদন ফরম'
                  : 'Membership Application'}
              </h2>
              <p className="text-xs text-on-surface-variant mt-0.5">
                {language === 'bn'
                  ? 'তারকা (*) চিহ্নিত ঘরগুলো অবশ্যই পূরণ করুন'
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label
                  htmlFor="nameBn"
                  className="font-label-md text-xs font-bold text-foreground block"
                >
                  {language === 'bn'
                    ? '১. সদস্যের নাম (বাংলা) *'
                    : '1. Name (Bangla) *'}
                ...
                <input
                  ...
                  id="nameBn"
                  name="nameBn"
                  className="w-full bg-surface dark:bg-background border border-border px-3.5 py-2.5 text-sm focus:outline-none focus:border-growth-green transition-colors text-foreground rounded-xl"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {language === 'bn' ? '(ঐচ্ছিক)' : '(Requierd)'}
                </label>
                <select
                  id="bloodGroup"
                  name="bloodGroup"
                  className="w-full bg-surface dark:bg-background border border-border px-3.5 py-2.5 text-sm focus:outline-none focus:border-growth-green transition-colors text-foreground cursor-pointer rounded-xl"
                >
                  <option value="">
                    {language === 'bn'
              ...
              <div className="space-y-1.5">
                <label
                  htmlFor="nidOrBirthReg"
                  className="font-label-md text-xs font-bold text-foreground block"
                >
                  {language === 'bn'
                    ? '১০. এনআইডি / জন্ম নিবন্ধন নম্বর'
                    : '10. NID / Birth Reg No'}
                  {language === 'bn' ? '(ঐচ্ছিক)' : '(Requierd)'}
                </label>
                <input
                  ...
                  <span className="text-xs font-bold text-growth-green">
                    {photoUrl
                      ? language === 'bn'
                        ...
                        : 'Change Photo'
                      : language === 'bn'
                        ? 'ছবি আপলোড করুন (সর্বোচ্চ ২ এমবি)'
