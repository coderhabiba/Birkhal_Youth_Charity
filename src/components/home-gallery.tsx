...
export function HomeGallery({
  ...
  const sectionHeading =
    (language === 'bn'
      ? settings['gallery_heading_bn']
      : settings['gallery_heading_en']) ||
    (language === 'bn'
      ? 'আমাদের কার্যক্রমের ছবি ও স্মৃতি অ্যালবাম'
      : 'Our Activity Moments & Gallery');
  const sectionDesc =
    (language === 'bn'
      ? settings['gallery_desc_bn']
      : settings['gallery_desc_en']) ||
    (language === 'bn'
  ...
  const allGalleryItems: GalleryItem[] = uniqueMedia.map((m: any) => {
    ...
    let catId: 'all' | 'activities' | 'relief' | 'education' | 'plantation' =
      'activities';
    if (
      rawTag.includes('relief') ||
      rawTag.includes('ত্রাণ') ||
      rawTag.includes('healthcare') ||
      rawTag.includes('স্বাস্থ্য') ||
      titleText.includes('ত্রাণ') ||
      titleText.includes('স্বাস্থ্য')
    ) {
      catId = 'relief';
    } else if (
      rawTag.includes('education') ||
      rawTag.includes('শিক্ষা') ||
      titleText.includes('শিক্ষা') ||
      titleText.includes('শিক্ষাসামগ্রী')
    ) {
      catId = 'education';
    } else if (
      rawTag.includes('plantation') ||
      rawTag.includes('বৃক্ষরোপণ') ||
      titleText.includes('বৃক্ষরোপণ') ||
    ...
    const rawTitle =
      m.title ||
      (language === 'bn'
        ? 'বীরখাল সমাজসেবা কার্যক্রম'
        : 'Birkhal Community Activity');
    const itemTitle =
      language === 'bn' ? rawTitle : translateBanglaBio(rawTitle, 'en');
    ...
    if (m.tag) {
      ...
      if (
        lowerTag.includes('স্বাস্থ্য') ||
        lowerTag.includes('medical') ||
        lowerTag.includes('চিকিৎসা')
      ) {
        tagBn = 'স্বাস্থ্যসেবা';
        tagEn = 'Healthcare';
      } else if (
        lowerTag.includes('শিক্ষা') ||
        lowerTag.includes('education')
      ) {
        tagBn = 'শিক্ষা সহায়তা';
        tagEn = 'Education Aid';
      } else if (
        lowerTag.includes('বৃক্ষরোপণ') ||
        lowerTag.includes('plantation') ||
        lowerTag.includes('সবুজায়ন')
      ) {
        tagBn = 'বৃক্ষরোপণ';
        tagEn = 'Plantation';
      } else if (
        lowerTag.includes('ত্রাণ') ||
        lowerTag.includes('relief') ||
        lowerTag.includes('বন্যা')
      ) {
        tagBn = 'ত্রাণ ও সেবা';
        tagEn = 'Relief & Aid';
      } else if (
        lowerTag.includes('যুব') ||
        lowerTag.includes('activity') ||
        lowerTag.includes('কার্যক্রম') ||
        lowerTag.includes('activities')
      ) {
        tagBn = 'যুব কার্যক্রম';
      ...
      {filteredItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-on-surface-variant bg-surface-container-lowest dark:bg-surface-container-low rounded-xl border border-dashed border-border">
          ...
          <p className="text-sm mt-1">
            {language === 'bn'
              ? 'অ্যাডমিন প্যানেল থেকে ছবি আপলোড করুন।'
            ...
            <div className="flex items-center justify-between p-4 mb-2">
              <div>
                <span className="px-3 py-1 bg-growth-green/90 text-white text-[11px] font-bold rounded-full mb-2 inline-block shadow-sm">
                  {language === 'bn'
                    ? selectedImage.categoryNameBn
                    : selectedImage.categoryNameEn}
                </span>
