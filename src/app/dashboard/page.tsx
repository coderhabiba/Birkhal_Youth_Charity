...
export default async function DashboardOverview() {
  ...
  const [
    ...
    DonationEntry.find({ status: 'Completed' }).select('amount date createdAt category donorName').lean(),
    Event.countDocuments({ status: 'upcoming' }),
    Review.countDocuments(),
    Member.find().sort({ createdAt: -1 }).limit(5).lean()
  ]);

  const totalDonations = completedDonations.reduce((sum: number, d: any) => sum + (d.amount || 0), 0);
