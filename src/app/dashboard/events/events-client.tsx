...
import { updateEventApprovalStatus } from '@/app/actions/events';
...
export function DashboardEventsClient({
  ...
  const [deleteTargetEvent, setDeleteTargetEvent] = useState<{
    id: string;
    title: string;
  } | null>(null);
  ...
  const confirmDeleteEvent = async () => {
    ...
    try {
      ...
      setErrorModal(
        language === 'bn'
          ? 'ইভেন্ট মুছে ফেলার সময় ত্রুটি ঘটেছে।'
  ...
  const handleApprovalStatusChange = (
    ...
    startTransition(async () => {
      await updateEventApprovalStatus(id, approvalStatus);
      setEvents(events.map(e => (e._id === id ? { ...e, approvalStatus } : e)));
    });
  };
  ...
  const filteredEvents = events.filter(ev => {
    ...
    const matchesApproval =
      approvalFilter === 'all' || ev.approvalStatus === approvalFilter;
    ...
    <>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          </p>          <div className="mt-2 flex gap-2 text-xs font-bold">
            <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400 px-2 py-1 rounded">
              {language === "bn" ? "পেন্ডিং: " : "Pending: "} {events.filter(e => e.approvalStatus === 'pending').length}
            </span>
            <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 px-2 py-1 rounded">
              {language === "bn" ? "অনুমোদিত: " : "Approved: "} {events.filter(e => e.approvalStatus === 'approved').length}
            </span>
          </div>
        </div>
        <button
      ...
      <div className="bg-surface-container-low dark:bg-surface-container-lowest p-4 border border-border flex flex-col md:flex-row gap-4 items-center justify-between mb-6 rounded-xl">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            placeholder={
              language === 'bn'
                ? 'ইভেন্টের নাম বা স্থান দিয়ে খুঁজুন...'
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-2 gap-2 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    <span
                      className={`text-xs font-bold px-2 py-0.5 uppercase tracking-wider rounded-md ${getStatusColor(event.status)}`}
                    >
                      {getStatusLabel(event.status)}
                    </span>
                    <span className={`text-xs font-bold px-2 py-0.5 uppercase tracking-wider rounded-md ${
                      event.approvalStatus === 'approved' ? 'bg-growth-green/10 text-growth-green' :
                      event.approvalStatus === 'rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                      'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400'
                    }`}>
                      {event.approvalStatus === 'approved' ? (language === 'bn' ? 'অনুমোদিত' : 'Approved') :
                       event.approvalStatus === 'rejected' ? (language === 'bn' ? 'বাতিল' : 'Rejected') :
                ...
                <div className="flex flex-col gap-1 mb-3 text-xs text-on-surface-variant">
                  <div className="flex items-center gap-1.5 font-bold">
                    <MapPin className="w-3.5 h-3.5 text-growth-green" />{' '}
                    {event.location}
                  </div>
              ...
              <div className="mt-auto flex flex-wrap gap-2 pt-3 border-t border-border">
                ...
                {event.approvalStatus !== 'approved' && (
                  <button
                    disabled={isPending}
                    onClick={() => handleApprovalStatusChange(event._id, 'approved')}
                ...
                {event.approvalStatus !== 'rejected' && (
                  <button
                    disabled={isPending}
                    onClick={() => handleApprovalStatusChange(event._id, 'rejected')}
      ...
      <ConfirmModal
        ...
        title={
          language === 'bn' ? 'ইভেন্ট মুছে ফেলার নিশ্চয়তা' : 'Delete Event'
        }
        message={
          language === 'bn'
            ? `আপনি কি নিশ্চিতভাবে "${deleteTargetEvent?.title}" ইভেন্টটি মুছে ফেলতে চান?`
            : `Are you sure you want to permanently delete "${deleteTargetEvent?.title}"?`
        }
